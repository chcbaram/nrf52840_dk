...
void fatfsCmdif(void)
{
...
#if _MAX_SS != _MIN_SS
        tot_sect = (fs->n_fatent - 2) * fs->csize * fs->ssize;
        fre_sect = fre_clust * fs->csize * fs->ssize;
#else
        tot_sect = (fs->n_fatent - 2) * fs->csize * _MIN_SS;
        fre_sect = fre_clust * fs->csize * _MIN_SS;
#endif
        /* Print the free space (assuming 512 bytes/sector) */
        cmdifPrintf("%10lu KiB total drive space.\n%10lu KiB available.\n", tot_sect/1024, fre_sect/1024);
      }
      else
      {
        cmdifPrintf(" err : ");
  ...
  else if (cmdifGetParamCnt() == 1 && cmdifHasString("mkfs", 0) == true)
  {
    cmdifPrintf("fatfs init \t: %d\n", is_init);
    ...
    if (is_init == true)
    {
      ...
      res = f_mkfs("", FM_FAT, 4096, buf, sizeof(buf));
      if (res != FR_OK)
      {
        cmdifPrintf("mkfs failed.\n");
      }
      else
      {
        cmdifPrintf("mkfs OK\n");
        ...
        if (fatfsMount() == true)
        {
          cmdifPrintf("mount OK\n");
        }
        else
        {
          cmdifPrintf("mount Fail\n");
        }
      }
    }
  }
